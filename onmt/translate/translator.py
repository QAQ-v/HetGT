...
class Translator(object):
    ...
    def translate(
        ...
        data = inputters.Dataset(
            self.fields,
            readers=([self.src_reader, self.tgt_reader, self.grh_reader]
                     if tgt else [self.src_reader, self.grh_reader]),
            data=[("src", src), ("tgt", tgt), ('grh', grh)] if tgt else [("src", src), ('grh', grh)],
    ...
    def _translate_batch(
        ...
        # batch middle
        for step in range(max_length):
            decoder_input = beam.current_predictions.view(1, -1, 1) # 1, batch*beam?, 1
