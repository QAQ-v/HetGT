...
from onmt.inputters.grh_dataset import grh_fields
...
def get_fields(
    ...
    grh_field_kwargs = {"vocab": edges_vocab}
    fields["grh"] = fields_getters["grh"](**grh_field_kwargs)
...
def _merge_field_vocabs(src_field, tgt_field, vocab_size, min_freq,
                        vocab_size_multiple, shared_vocab_size):
    ...
    merged_vocab = Vocab(
        ...
        max_size=int(shared_vocab_size), min_freq=min_freq
    )
...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
        ...
        self.yield_raw_example = yield_raw_example
        self.pool_factor = pool_factor
        # Lazy iter应该指的是每次在__iter__里load, 此处改成非lazy, 即声明Iter对象时就把data load进内存里不释放
        for path in dataset_paths:
            self.cur_dataset = torch.load(path)
            logger.info('Loading dataset from %s' % path)
            logger.info('number of examples: %d' % len(self.cur_dataset)) # 为什么train的logger不打印在console?
    ...
    def _iter_dataset(self, path):
        ...
        cur_iter = OrderedIterator(
            ...
            repeat=False, # 注意这里repeat是false! 改成true之后则很多代码就可以省略了, 感觉就不需要lazyDataIter这个类了
            yield_raw_example=self.yield_raw_example
        )
        for batch in cur_iter: #感觉这样写是为了多shards好?
