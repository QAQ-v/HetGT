...
def model_opts(parser):
    ...
    # graph options
    group.add('--aggregation', '-aggregation', type=str, default=None,
              choices=['jump', 'dense', None],
              help="Type of aggregation method of encoder layer output")
    group.add('--gate', '-gate', action='store_true',
              help="If use gate")
    group.add('--edges', '-edges', type=int, default=3,
              help="num of edge types")
...
def preprocess_opts(parser):
    ...
    group.add('--train_graph', '-train_graph', required=True, nargs='+',
              help="Path(s) to the train graph edges data")
    group.add('--valid_graph', '-valid_graph',
              help="Path(s) to the valid graph edges data")
    group.add('--edges_vocab', '-edges_vocab', required=True,
              help="Path(s) to the valid graph edges data")
    group.add('--shared_vocab_size', '-shared_vocab_size',
              help="shared_vocab_size")
...
def translate_opts(parser):
    ...
    # graph
    group.add('--grh', '-grh', required=True,
              help="Source sequence graph")
    group.add('--gate', '-gate', action='store_true',
              help="If use gate")
