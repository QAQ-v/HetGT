...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
                 gate, edges, max_relative_positions=0):
        super(TransformerEncoderLayer, self).__init__()

        self.self_attn = MultiHeadedAttention(
            heads, d_model, dropout=attention_dropout,
            max_relative_positions=max_relative_positions,
            edge_type=edges, gate=gate)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)
        self.dropout = nn.Dropout(dropout)

    def forward(self, inputs, grh, mask):
        """
        Args:
        ...
        context, _ = self.self_attn(input_norm, input_norm, input_norm, grh,
    ...
    def __init__(self, num_layers, d_model, heads, d_ff, dropout,
        ...
        self.transformer = nn.ModuleList(
            [TransformerEncoderLayer(
                ...
                gate, edges, max_relative_positions=max_relative_positions)
        ...
        if aggregation is not None:
            if aggregation == "dense":
                self.dense_linear = nn.ModuleList(
                    [nn.Linear(d_model+i*d_model, d_model) for i in range(num_layers)]
                )
                ## TODO: MAX_POOLING, LSTM
                self.aggregate_linear = nn.Linear((1+num_layers)*d_model, d_model)
                self.aggregate_layer_norm = nn.LayerNorm((1 + num_layers) * d_model, eps=1e-6)

            if aggregation == "jump":
                ## TODO: MAX_POOLING, LSTM
                self.aggregate_linear = nn.Linear((num_layers+1)*d_model,
                                                  d_model)
                self.aggregate_layer_norm = nn.LayerNorm((1 + num_layers) * d_model, eps=1e-6)
