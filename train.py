...
def main(opt):
    ...
    if opt.world_size > 1:
        ...
        procs = []
        for device_id in range(nb_gpu): # 建立了多个管道q 每个q对应以一个iter 不断产生batch
        ...
        producer = mp.Process(target=batch_producer, # 运行了上面那些子线程后 再运行产生batch的线程 使其中的q不断产生batch
...
def batch_producer(generator_to_serve, queues, semaphore, opt):
    ...
    b = next_batch(0) # generator_to_serve这个就是前面建立的train_iter 所以所有线程的batch都由一个iter产生
